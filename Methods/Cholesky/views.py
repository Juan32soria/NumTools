...
from .forms import CholeskyForm  # Formulario específico para Cholesky
...
def cholesky_view(request):
    ...
    if request.method == 'POST':
        ...
        form = CholeskyForm(request.POST, matrix_size=matrix_size)
        ...
        if form.is_valid():
            try:
                ...
                solutions, errors, final_approximation, iterations = nm.cholesky(
                    matrix, b, tol, niter
                )
                ...
                context.update({
                    ...
                    'graph': img_base64,
                    'errors': errors,
                    'msg': ['Cholesky method completed successfully.'],
                })
        ...
        form = CholeskyForm(matrix_size=matrix_size)
    ...
    return render(request, 'cholesky.html', context)
